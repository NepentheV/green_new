...
flags.DEFINE_integer('numHist', 0, 'numHist')   # 128
# gnn
flags.DEFINE_bool('item_emb', True, 'item_emb')
...
flags.DEFINE_float('gcl_w', 0.01, 'The weight of DGI obj.')

# user_emb
flags.DEFINE_bool('user_emb', True, 'user_emb')
flags.DEFINE_bool('gnn_user_emb', True, 'gnn_user_emb.')
...
print('numHist:'+str(FLAGS.numHist))
...
if FLAGS.numHist==0:
    ...
    datas = '../dataset'+str(FLAGS.numHist)+'.pkl'  #../dataset20.pkl
...
if FLAGS.numHist == 0:
    remaps = '../raw_data/remap.pkl'
else:
    remaps = '../raw_data/remap'+str(FLAGS.numHist)+'.pkl'   #../raw_data/remap20.pkl
...
# full_indices = tf.cast(tf.stack([locs[:, 0], locs[:, 2]], axis=1), tf.int64)
# ids_in_sp = tf.SparseTensor(full_indices, tf.cast(vals, tf.int64), tf.constant(size, dtype=tf.int64))
# weights_in_sp = tf.SparseTensor(full_indices, tf.cast(weights, tf.int64), tf.constant(size, dtype=tf.int64))

#################################################################

# full_indices = tf.cast(tf.stack([locs[:, 0], locs[:, 1]], axis=1), tf.int64)
# adj_in_sp = tf.SparseTensor(full_indices, tf.cast(np.ones_like(weights), tf.int64), tf.constant((item_count, item_count), dtype=tf.int64))

# d_in_sp = tf.sparse_reduce_sum_sparse(adj_in_sp, axis=0)
# d_adj_in_sp = tf.SparseTensor(([i,i] for i in range(item_count)), d_in_sp.values, tf.constant((item_count, item_count), dtype=tf.int64))
