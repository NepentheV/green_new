...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list, support_in_sp, support_in_sp1):
        ...
        myhist_emb = tf.concat([
            tf.nn.embedding_lookup(item_emb_w, self.hist_i),
            tf.nn.embedding_lookup(cate_emb_w, hc),
        ], axis=2)

        rnn_outputs, final_output = dynamic_rnn(GRUCell(hidden_units), inputs=myhist_emb,
                                     # sequence_length=self.seq_len_ph,
                                     dtype=tf.float32,
                                     scope="gru1")

        # hist_i = attention(i_emb, myhist_emb, self.sl)
        hist = attention(final_output, rnn_outputs, myhist_emb, self.sl)
        hist = tf.layers.batch_normalization(inputs=hist)
        hist = tf.reshape(hist, [-1, hidden_units], name='hist_bn')
        hist = tf.layers.dense(hist, hidden_units, name='hist_fcn')


        VA_i = tf.multiply(i_emb, final_output)
        VA_j = tf.multiply(j_emb, final_output)

   
        if FLAGS.user_emb:
            din_i = tf.concat([hist, i_emb, user_emb, final_output, VA_i], axis=-1)
        else:
            din_i = tf.concat([hist, i_emb, final_output, VA_i], axis=-1)
        ...
        if FLAGS.user_emb:
            din_j = tf.concat([hist, j_emb, user_emb, final_output, VA_j], axis=-1)
        else:
            din_j = tf.concat([hist, j_emb, final_output, VA_j], axis=-1)
